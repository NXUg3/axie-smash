...
export class Renderer{
  ...
  drawFighter(f){const ctx=this.ctx,sprite=this.assets.sprites[f.type],cached=frame(sprite,f.spriteState,f.spriteFrame,f.type),scale=130/cached.height,cx=f.x+f.w/2;
    ...
    ctx.save();ctx.translate(cx,f.y);ctx.scale(f.facing,1);ctx.imageSmoothingEnabled=false;ctx.globalAlpha=f.hurtFlash%2?.55:1;ctx.drawImage(cached.canvas,-(cached.width/2+cached.pad)*scale,-(cached.height+cached.pad)*scale,cached.canvas.width*scale,cached.canvas.height*scale);ctx.restore();
  }

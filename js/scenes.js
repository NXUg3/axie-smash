...
export class Scenes{
  ...
  menu(name){const app=this.app,t=app.t,host=this.host;host.classList.add('menu');host.append(el('h1',t(name==='root'?'menu':name)));const list=group('menu-list');
    ...
    for(const id of menus[name]){const b=button(t(id),()=>open(id),blocked.has(id));if(blocked.has(id))b.append(el('span',t('soon'),{className:'soon'}));list.append(b);}host.append(list);
